...
public class TransformerArcaneWorkbench implements Opcodes {
    ...
    public static byte[] transformGui(byte[] origCode) {
        ...
        ThaumicWandsRemastered.logger.log(Level.INFO, "[CORE] Patching Arcane Workbench");
        ...
        ClassReader cr = new ClassReader(origCode);ClassNode classNode = new ClassNode();
        ...
        for(MethodNode methodNode : classNode.methods)
            if(methodNode.name.equals(methodToPatch)) {
                ...
                endList.add(new MethodInsnNode(INVOKESTATIC, "com/interplay/thaumicwandsremastered/asm/TransformerArcaneWorkbench", "b_onBlockActivated", desc, false));
    ...
    public static byte[] transformTile(byte[] origCode) {
        ...
        instrcutions.add(new MethodInsnNode(INVOKESTATIC, "com/interplay/thaumicwandsremastered/asm/TransformerArcaneWorkbench", "t_createNewTileEntity", desc, false));
    ...
    public static boolean b_onBlockActivated(World world, BlockPos pos, EntityPlayer player) {
        if(world.isRemote)
            return true;
        player.openGui(ThaumicWandsRemastered.instance, TWR_GuiHandler.guiArcaneWorkbench, world, pos.getX(), pos.getY(), pos.getZ());
        return false;
    }
