...
import com.interplay.thaumicwandsremastered.util.WandWrapper;
...
public class RecipeWand extends IForgeRegistryEntry.Impl<IRecipe> implements IPlayerDependentArcaneRecipe {
    ...
    private WandWrapper getParts(InventoryCrafting inv) {
        ...
        for(IWandCap wc : ThaumicWandsRemasteredAPI.wandCaps.values())
            if(ItemStack.areItemsEqual(inv.getStackInSlot(2), wc.getItemStack())) {
                wrapper.setCap(wc);
                break;
            }

        for(IWandRod wr : ThaumicWandsRemasteredAPI.wandRods.values())
